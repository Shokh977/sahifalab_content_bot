...
from bot.drafting import news_drafter, quote_drafter, tip_drafter, youtube_drafter
from bot.ingestion import rss, youtube
...
async def job_ingest_and_draft(bot: Bot) -> int:
    ...
    for item in new_items:
        try:
            ...
            if source and source["kind"] == "youtube":
                draft_id = await youtube_drafter.draft_from_item(pool, item)
            else:
                draft_id = await news_drafter.draft_from_item(pool, item)
